...
def sample_mask(sample_sizes, max_size):
    ...
    presence = [
        torch.randperm(max_size).to(device) < sample_sizes[b]
        for b in range(sample_sizes.size(0))
    ]
...
def get_mask(sizes, max_size):
    ...
    device = sizes.device
    presence = [
        torch.arange(max_size).to(device) < sizes[b] for b in range(sizes.size(0))
    ]
    mask = ~torch.stack(presence, dim=0).to(device)
    return mask.bool()


def masked_fill(tensor_bnc, mask_bn=None, value=0.0):
    return (
        tensor_bnc.masked_fill(mask_bn.unsqueeze(-1), value)
...
def check(x):
    ...
    assert not (
        isinf or isnan
    ), f"Tensor of shape [{x.shape}] is isinf:{isinf} or isnan:{isnan}"
